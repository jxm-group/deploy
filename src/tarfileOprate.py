...
def get_workspace_files():
    ...
    if  SHOW_LOG:
        print('获取工作空间下的所有文件...')
    # return os.listdir('./')
    os.chdir('F:\\testTar\\Directx_Redist_x64')
    return os.listdir('F:\\testTar\\Directx_Redist_x64')
...
def extract_all_files(tar_path, ext_path):
    '''解压tar文件中的部分文件到指定目录中'''
    print "解压------",tar_path
    with tarfile.open(tar_path) as tar:
        if SHOW_LOG:
...
def main():
    ...
    # extract_all_files(TAR_PATH, EXT_PATH)
